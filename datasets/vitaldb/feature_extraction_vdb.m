...
fs = 125;
win_size = 60;
passed_path = 'passed_vitaldb_1m.csv';
gen_path = 'features_vitaldb_1m.csv';
load('gendata/filter_coefs.mat');
%%
feature_table = table;
prevOutcome = readtable(sprintf('generated/%s',passed_path));
ct = parallel.pool.Constant(prevOutcome);
demogData = readtable('vitaldb/demog.csv');
demogData_const = parallel.pool.Constant(demogData);
...
parfor(r=1:size(records, 1),8)
    ...
    case_name = split(record, '.', 1);
    case_name = str2double(case_name(1));
    demog_inst = demogData_const.Value;
    demog_inst = demog_inst(demog_inst.caseid == case_name, :);

    record_path = sprintf('vitaldb/data/%s', char(record));
    ...
    abp_flt = filter(abp_coefs, 1, abp_raw);
    ...
    for oc = 1:size(occs, 1)
        ...
        % demographics
        fr{1, 'DEMG_AGE'} = demog_inst{1, 'age'};
        fr{1, 'DEMG_BMI'} = demog_inst{1, 'bmi'};
        fr{1, 'DEMG_SEX'} = double(strcmp(demog_inst{1, 'sex'}, 'F'));
        fr{1, 'DEMG_HEIGHT'} = demog_inst{1, 'height'};
        fr{1, 'DEMG_WEIGHT'} = demog_inst{1, 'weight'};
        ...
        % mixed demog-ppg features

        % stiffness index (height/deltaT)
        fr{1, 'MIX_STIFDX'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_DT'};
        fr{1, 'MIX_WEIGHT_DT'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_DT'};
        fr{1, 'MIX_BMI_DT'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_DT'};

        fr{1, 'MIX_HEIGHT_T1'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_T1'};
        fr{1, 'MIX_WEIGHT_T1'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_T1'};
        fr{1, 'MIX_BMI_T1'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_T1'};

        fr{1, 'MIX_HEIGHT_T2'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_T2'};
        fr{1, 'MIX_WEIGHT_T2'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_T2'};
        fr{1, 'MIX_BMI_T2'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_T2'};

        fr{1, 'MIX_HEIGHT_T3'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_T3'};
        fr{1, 'MIX_WEIGHT_T3'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_T3'};
        fr{1, 'MIX_BMI_T3'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_T3'};

        fr{1, 'MIX_HEIGHT_TPI'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_TPI'};
        fr{1, 'MIX_WEIGHT_TPI'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_TPI'};
        fr{1, 'MIX_BMI_TPI'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_TPI'};

        fr{1, 'MIX_HEIGHT_TPP'} = fr{1, 'DEMG_HEIGHT'} / fr{1, 'PPG_TPP'};
        fr{1, 'MIX_WEIGHT_TPP'} = fr{1, 'DEMG_WEIGHT'} / fr{1, 'PPG_TPP'};
        fr{1, 'MIX_BMI_TPP'} = fr{1, 'DEMG_BMI'} / fr{1, 'PPG_TPP'};
